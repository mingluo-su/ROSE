...
def get_mmlu_prox_lite(nsamples, seed, seqlen, tokenizer):
    dataset = load_dataset('li-lab/MMLU-ProX-Lite', "zh", split='test')
    ...
    def build_prompt(example, lang_dict):
    ...
    max_opt_num = 10
    samples = dataset.select(range(nsamples))
    calibration_data = []

    for example in samples:

        prompt = build_prompt(example,lang_dict)
        trainenc = tokenizer(prompt, return_tensors='pt', padding="max_length",max_length=2048,truncation=True)
        # i = random.randint(0, trainenc.input_ids.shape[1] - seqlen - 1)
        # j = i + seqlen
...
def get_loaders(
    ...
    if "mmlu" in name:
        return get_mmlu_prox_lite(nsamples, seed, seqlen, tokenizer)
