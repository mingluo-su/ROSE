...
from lib.utils_module import check_sparsity
...
def main():
    ...
    parser.add_argument("--sparsity_type", type=str, default="unstructured", choices=["unstructured", "4:8", "2:4"], help='Type of sparsity pattern: unstructured or structured (e.g., 4:8, 2:4).')
    ...
    parser.add_argument("--tasks", type=str, nargs="+", default=["winogrande","boolq","piqa","openbookqa","hellaswag","arc_easy","arc_challenge"], help="List of evaluation tasks (space-separated).")
    ...
    sparsity = getattr(args, 'sparsity_ratio', 0.0)              
    ...
    print(f"ppl on wikitext2: {ppl_wikitext}")

    col_width = 15
    ppl_header_items = ["Model", "Sparsity", "Method", "Dataset", "PPL"]
    ...
    if args.eval_zero_shot:
        ...
        acc_filename = f"results/acc/{args.model}.txt"

        print("zero_shot evaluation results")

        metric_vals = eval_zero_shot(model, tokenizer, tasks=args.tasks)
        metric_keys = list(metric_vals.keys())  # 🔥 自动获取 key
        ...
        data_items = [model_name, f"{sparsity:.1%}", prune_method] + values
